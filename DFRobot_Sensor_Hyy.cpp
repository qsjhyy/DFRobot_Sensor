...
int DFRobot_Sensor_Hyy::begin(void)
{
	...
	if(0 == readReg(SENSOR_ADDR_ID, &buf, 1))		//判断数据总线是否成功
	{
...
uint32_t DFRobt_Sensor_Hyy::lightStrengthLux(void)
{
...
DFRobot_Sensor_Hyy_IIC::DFRobot_Sensor_Hyy_IIC(TwoWire *pWire, uint8_t mode)
	:DFRobot_Sensor(mode)		//初始化列表：子类用父类给的构造函数从而初始化了父类的私有变量_mode
{
	——deviceAddr = DFRobot_Sensor_IIC_ADDR;
	_pWire = pWire;
}
...
int DFRobot_Sensor_Hyy_IIC::begin(void)
{
	...
	return DFRobot_Sensor::begin();		//利用父类的初始化函数
}
...
void DFRobot_Sensor_Hyy_SPI::writeReg(uint8_t reg, void* pBuf, size_t size)
{
	...
	_pSpin->beginTransaction(SPISettings(14000000, MSBFIRST, SPI_MODE0));
